...
async function fetchTexas(params: SearchBody): Promise<{ opportunities: NormalizedOpportunity[]; status: ProviderStatus; totalRecords: number }> {
  ...
  let opportunities: NormalizedOpportunity[] = allItems.map((opp: any) => ({
    external_id: String(opp.po_contract_number || opp.rfo_number || opp.contract_number || opp.id || Math.random().toString(36).slice(2)),
    source: "texas_smartbuy",
    title: opp.project_name || opp.rfo_description || opp.vendor_name_description || opp.primary_vendor_name || opp.description || "",
    solicitation_number: String(opp.po_contract_number || opp.rfo_number || opp.contract_number || ""),
    department: opp.agency_name || opp.agency || opp._tx_dataset || "Texas",
    naics_code: opp.pcc_code || opp.nigp_code || opp.commodity_code || "",
    posted_date: opp.start_date || opp.contract_start || opp.effective_date || opp.award_date || null,
    response_deadline: opp.end_date || opp.contract_termination_date || opp.expiration_date || null,
    set_aside: opp.primary_vendor_hub_status || opp.hub_status || opp.set_aside || "",
    description_url: opp.url || "https://www.txsmartbuy.com/esbd",
    type: opp.contract_type || opp.contract_subtype || opp.contract_or_po_2 || opp.type || "contract",
    raw_data: opp,
    resource_links: [],
    description_text_url: null,
  }));
