-- Phase 2: Enterprise Subscription & Team Management (Fixed Dependencies)

-- Drop existing policies that depend on the role column
DROP POLICY IF EXISTS "Organization owners can manage subscription" ON public.organization_subscriptions;
DROP POLICY IF EXISTS "Organization owners can manage subscriptions" ON public.organization_subscriptions;
...
-- Update organization_members to use the new role type with proper conversion
ALTER TABLE public.organization_members 
DROP CONSTRAINT IF EXISTS organization_members_role_check,
ADD COLUMN new_role organization_role_type;

-- Convert existing roles to new enum type
UPDATE public.organization_members 
SET new_role = CASE 
  WHEN role = 'owner' THEN 'owner'::organization_role_type
  WHEN role = 'admin' THEN 'admin'::organization_role_type  
  WHEN role = 'member' THEN 'editor'::organization_role_type
  ELSE 'viewer'::organization_role_type
END;

-- Drop old role column and rename new one
ALTER TABLE public.organization_members 
DROP COLUMN role CASCADE,
ADD COLUMN role organization_role_type DEFAULT 'viewer'::organization_role_type;

UPDATE public.organization_members SET role = new_role;
ALTER TABLE public.organization_members DROP COLUMN new_role;
ALTER TABLE public.organization_members ALTER COLUMN role SET NOT NULL;

-- Add additional columns for enhanced team management
ALTER TABLE public.organization_members 
...
-- Recreate the dropped policies with updated references
